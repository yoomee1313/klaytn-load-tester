...
import (
	...
	"github.com/klaytn/klaytn-load-tester/contracts/internalTx"
	"log"
	"math/big"
	"math/rand"
	"sync"
...
func deployContracts(coinbase *account.Account) {
	...
	for {
		var err error
		KIP17Address, KIP17Tx, KIP17Contract, err = internalTx.DeployKIP17TokenContract(auth, conn)
		if err == nil {
	...
	for {
		var err error
		mainAddress, mainTx, mainContract, err = internalTx.DeployMainContract(auth, conn, KIP17Address)
		if err == nil {
			coinbase.UpdateNonce()
			break
		}
