...
import (
	...
	"github.com/klaytn/klaytn-load-tester/contracts/blockbench/cpuHeavyTC"
...
func deployContract(coinbase *account.Account, endPoint string) {
	...
	for {
		addr, tTx, cpuHeavy, err := cpuHeavyTC.DeployCPUHeavy(auth, conn)
		address = addr
		tx = tTx
		if err != nil {
